...
Les couches **Nerfs**, **Organes** et **Vaisseaux sanguins** sont **trois interrupteurs distincts** (jamais fusionnés). Les nerfs périphériques majeurs absents de BodyParts3D 4.0 (sciatique, médian, plexus, etc.) sont des tubes `synthetic-v3+`. Le cœur-organe et les coronaires ne partagent pas la même couche.
...
Le script squelette télécharge `isa_BP3D_4.0_obj_99.zip`, `isa_element_parts.txt` (correspondance concept → fichier `FJ####`) et les listes IS-A depuis l’archive DBCLS, garde les **feuilles** descendantes de `FMA5018` (bone organ) plus le sternum, y ajoute le **cartilage** BodyParts3D disponible (cartilages costaux gauches/droits 1–7, disques intervertébraux nommés, cartilage thyroïde / cricoïde / aryténoïdes / corniculés / cunéiformes, épiglotte — fichiers FJ uniques ; **cartilages nasaux exclus**), puis écrit `public/models/skeleton.glb` (nœuds `bones` + `cartilage`) et `src/data/landmarks.json` (os seulement). BodyParts3D 4.0 n’a pas de cartilage articulaire hyalin ni de ménisques. Pour retirer le nez d’un GLB déjà construit : `python3 scripts/strip_nasal_cartilage.py`.
